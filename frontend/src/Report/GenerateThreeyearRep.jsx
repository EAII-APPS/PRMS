...
function GenerateThreeYearReportTable() {
  ...
  const [selectedYear, setSelectedYear] = useState(currentYear - 7);
  ...
  const downloadTableAsPDF = async () => {
    try {
      ...
      const response = await axiosInistance.get("/planApp/threepdf_performance/", {
        headers: {
        ...
        params: {
          ...
          sector: selectedSector,
          division: selectedDivision,
          quarter: quarterMapping[selectedQuarter],
        },
  ...
  const downloadTableAsWord = async () => {
    try {
      const response = await axiosInistance.get("/planApp/threeword_performance/", {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("access")}`,
        },
        params: {
    ...
    <>
      <div className="grid gap-3 items-center">
        <h1 className="whitespace-nowrap text-left text-xl font-bold text-black">
          {t("MAIN.SIDEBAR.PLAN.GENERATE_PLAN_TABLE.TITLETHREEYEARREPORT")}
        </h1>
                                  ...
                                  <div className="flex justify-center gap-0">
                                    {(!selectedQuarter || selectedQuarter === 1) && (
                                      <td className="py-4  text-left pl-4 text-sm  text-black w-full tracking-wider  border-[#535353]">
                                        {kpi.performance_data.year_one_performance ? kpi.performance_data.year_one_performance : "N/A"}
                                      </td>
                                    )}
                                    {(!selectedQuarter || selectedQuarter === 2) && (
                                      <td className="py-4  text-left pl-4 text-sm  text-black w-full tracking-wider  border-[#535353]">
                                        {kpi.performance_data.year_two_performance ? kpi.performance_data.year_two_performance : "N/A"}
                                      </td>
                                    )}
                                    {(!selectedQuarter || selectedQuarter === 3) && (
                                      <td className="py-4  text-left pl-4 text-sm  text-black w-full tracking-wider  border-[#535353]">
                                        {kpi.performance_data.year_three_performance ? kpi.performance_data.year_three_performance : "N/A"}
                                      </td>
                                    )}
                                    {(!selectedQuarter || selectedQuarter === 3) && (
                                      <td className="py-4  text-left pl-4 text-sm  text-black w-full tracking-wider  border-[#535353]">
                                        {kpi.performance_data.total_performance ? kpi.performance_data.total_performance : "N/A"}
                                      </td>
                                    )}
                                  </div>
                                </td>
...
export default GenerateThreeYearReportTable;

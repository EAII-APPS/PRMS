...
function Sector() {
  ...
  const handleAddSector = async (e) => {
    ...
    if (!namex) {
      setErrorEmptyMessage("please enter sector");
      return;
    }
    if (!email) {
  ...
  const handleOpenDelete = (sectorId, selectedName) => {
  ...
  const handleDelete = async () => {
    dispatch(deleteSelectedSectorData(selectedId));
  ...
  const handleEdit = async (e) => {
    ...
    try {
      ...
      toast.success(`Sector edited successfully`, {
        autoClose: 2000,
      });
  ...
  useEffect(() => {
    const calculateTotalPages = () => {
      setTotalPages(
        Math.ceil(sectorData ? sectorData.length / itemsPerPage : [])
      );
    };
    calculateTotalPages();
  }, [sectorData, itemsPerPage]);
  ...
  const currentPageData = sectorData
    ? sectorData.slice(indexOfFirstItem, indexOfLastItem)
    : [];
    ...
    <>
                    ...
                    <div className="flex shrink-0 flex-col gap-2 sm:flex-row">
                      {authInfo.user.userPermissions.includes(
                        ...
                        >
                          ...
                          {t("MAIN.SIDEBAR.CLUSTER.SECTOR.ADDBUTTON")}
                        </Button>
                      ) : (
                        <div></div>
                      )}
      ...
      <Dialog open={open} handler={handleOpen} size="sm">
        <DialogHeader className="flex justify-between">
          <div className="text-xl ml-5">{t("MAIN.INPUTFIELD.ADD_SECTOR")}</div>
      ...
      <Dialog open={openEdit} handler={handleOpenEdit} size="sm">
        <DialogHeader className="flex justify-between">
          <div className="text-xl ml-5">
            {t("MAIN.INPUTFIELD.UPDATE_SECTOR")}
          </div>
